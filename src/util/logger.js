...
log4js.configure({
    appenders:{
        ...
        log_file:{//记录器2：输出到文件
            type : 'file',
            filename: process.cwd() + `/logs/${fileName}.log`,//文件目录，当目录文件或文件夹不存在时，会自动创建
            maxLogSize : 104857600,//文件最大存储空间（byte），当文件内容超过文件存储空间会自动生成一个文件test.log.1的序列自增长的文件
        ...
        //     type: "dateFile",
        //     filename: process.cwd() + `/logs/${fileName}`,//您要写入日志文件的路径
        ...
        //     type: "file",
        //     filename: process.cwd() + `/logs/${fileName}_error`,//您要写入日志文件的路径
        //     alwaysIncludePattern: true,//（默认为false） - 将模式包含在当前日志文件的名称以及备份中
        //     daysToKeep:10,//时间文件 保存多少天，距离当前天daysToKeep以前的log将被删除
        //     //compress : true,//（默认为false） - 在滚动期间压缩备份文件（备份文件将具有.gz扩展名）
    ...
    categories: {
        default:{appenders:['console', 'log_file'], level: 'warn' },//默认log类型，输出到控制台 log文件 log日期文件 且登记大于info即可
        production:{appenders:['log_file'], level:'warn'},  //生产环境 log类型 只输出到按日期命名的文件，且只输出警告以上的log
        console:{appenders:['console'], level:'debug'}, //开发环境  输出到控制台
        debug:{appenders:['console', 'log_file'], level:'debug'}, //调试环境 输出到log文件和控制台
        error:{appenders:['console', 'log_file'], level:'error'}//error 等级log 单独输出到error文件中 任何环境的errorlog 将都以日期文件单独记录
    },
});
